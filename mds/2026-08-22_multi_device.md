...
## 次にやること
1. 修正版で再度実機テスト(キーボード直結 + Hub(マウス+キーボード)の組み合わせ)
2. それでも`No more HCD channels available`が出る場合、構成(デバイス数・各デバイスのインターフェース数)を教えてもらって追加調査
3. 9ボタンマウスの6番目以降のボタンを本当に使いたい場合は、素のボタン拡張(3箇所修正)かfilter_rules.hでのショートカット変換か、方針を決める
