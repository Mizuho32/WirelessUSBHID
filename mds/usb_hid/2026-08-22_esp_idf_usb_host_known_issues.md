...
`mds/usb_hid/2026-08-22_rp2040_host_check.md`でRP2040クロステストにより、9ボタンマウス(Maxxterドングル)の3バイート切り詰め問題は**ドングル側ではなくESP32-S3のUSBホストコントローラか`espressif/usb_host_hid`ドライバ(ESP-IDF側)固有の問題**と確定した。現在`IDF5.5.1`が最新ではないので、既知issue・アップデートで直る可能性を軽く調査した。
