...
- ~~BLE有効時、WebUIの「保存」がまれにOOMで失敗する~~ → 解決済み([2026-09-09_ble_webui_syntax_check_oom.md](2026-09-09_ble_webui_syntax_check_oom.md)参照)。NimBLEの常駐フットプリントではなく、mrubyのPrismパーサ+codegenのピークメモリ需要がBLE常駐後の残りSRAMを超えることが真因と判明。syntax-check用の使い捨てVMだけPSRAMへ逃がす対策で実機確認済み。
