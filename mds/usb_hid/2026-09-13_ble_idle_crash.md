...
### 追記2: `start:`/`stop:`ログだけでは弱い - 定期詳細ログを追加

`start:`/`stop:`ログはBLEのトグルというイベントに紐付いた前後2点のスナップショットに過ぎず、「リークしてるのはほぼ確実」という前提に対して「じゃあ何が/いつ/どのくらいのペースで」を追う手段としては弱い(BLE ONのまま長時間放置するケースは1点もログが出ない、9時間の間に何が起きたかの時系列も分からない)、という指摘を受けて`heap_monitor.c`を追加した:

- `app_main()`の一番最初(`status_led_init()`の直後、他の何もまだ動いていない時点)で起動する`esp_timer`が、30分おきに永久にログを出し続ける - BLEのON/OFFと無関係、常時稼働。
- 単純な空きバイト数だけでなく`heap_caps_get_info()`の内訳一式を出す: `total_free_bytes`(空き)、`largest_free_block`(確保可能な最大連続サイズ - 断片化の指標)、`minimum_free_bytes`(起動以来の最低記録 - 30分間隔のサンプル点の間に起きた一時的な落ち込みも取りこぼさない)、`allocated_blocks`(確保中のブロック数 - これが単調増加していれば「都度何かをmallocして無くしている」典型的リークの証拠、増えずに空きだけ減っていくなら断片化寄り)、`total_allocated_bytes`。
- 既存の`start:`/`stop:`ログ(イベント駆動、BLEトグルの瞬間が分かる目印)はそのまま残し、この定期ログと突き合わせられるようにしてある。

次にBLE ON状態のまま長時間放置するケースと、BLE OFFのまま長時間放置するケースの両方でこの定期ログを取れば、「BLEが動いてる間だけ減るのか」「切っていても関係なく減るのか」「そもそもリークなのか断片化なのか」がこのログだけで判別できるはず。

### 追記3: 実際に9時間超放置→BLE ONを再現、クラッシュせず - ただし原因は未確定のまま区切り

`heap_monitor`導入後、実機で元の状況(`ble_toggle false`→無操作のまま放置)を再度、今度は約9.8時間再現した。30分おきの定期ログを終始追跡した結果:

- 起動〜30分: 初期化コストで265540B→100384B free まで低下(既知)。
- 30分〜300分(4.5時間): free=99680〜100456のレンジ内で小さく増減するのみ。`largest_block`(45056)/`min_ever_free`(22376)は完全に不動。
- 330分: free=95184(-4496B)、`allocated_blocks`+102 -唯一の大きな階段状変化。原因不明(WiFiの定期フルスキャン/DHCPリース更新/NTP再同期など数時間おきのイベントを疑ったが未特定)。
- 360分〜540分(9時間): 330分の値のままピクリとも動かず、8回連続完全一致。継続的なリークではなく単発の階段状消費だったと判断できる。
- 540分(9時間)の節目: クラッシュなし、ヒープも健全(free=95184のまま)。
- 586.5分(9.8時間)、実際に`ble_toggle true`を実行 → **クラッシュせず正常に起動・接続成功**(直前のfree=92072、largest_block=45056で、危険域には程遠かった)。その後何度かtoggleを繰り返しても問題なし(最後の`stop`で`largest_block`が40960まで落ちた - この観測期間中で唯一の断片化の兆候だが、実害が出るレベルではない)。

**結論(正直な評価)**: 今回のテストでは「9時間BLE off無操作放置→BLE ON」の再現条件を満たしたが、クラッシュは再現しなかった。上記(追記なし本文)で直したuuid16リーク修正が効いたのかは実は証明も反証もできていない - あのリークは1サイクルあたり数バイト程度しかなく、そもそも今回のテストは放置前のBLE startが1回だけ(元のクラッシュ時は2回起きてから3回目で落ちた)で、条件が完全には一致していない。ヒープモニタの実測でも、9時間の純粋な無操作放置だけではfreeは92KB台までしか下がらず、`ble_gattc_init`のENOMEMを起こすような危険域には全く届かなかった。

つまり「今回は危険域に達しなかったので、そもそも直っていたかどうかを判定できる状況ではなかった」というのが一番正直な結論。**元のクラッシュの真の原因(何が内部ヒープをあそこまで圧迫したか)は依然として未確定**。ただし:

- 実バグ(uuid16リーク)を1つ見つけて修正できた(直す価値はあった)。
- `heap_monitor.c`という恒久的な診断手段が残った - 次に同様の現象が起きた際、より詳細な手がかりが得られる。
- 少なくとも「素の無操作放置」だけでは再現しないと分かった - 再発時は他の要因(WiFi環境の変化、より多いtoggleサイクル数、実際のキーボード/マウス使用を伴う放置など)との組み合わせを疑う。
